...
const contenedorTareasTrz = document.getElementById('contenedor-tareas-trz');
 

// Contador global para generar IDs únicos
let contadorTareasTrz = 1;

// FUNCIÓN PRINCIPAL PARA CREAR TAREAS

function crearTareaTrz(id, autoFocus = false) {
    ...
    if (id % 2 === 1) {
        ...
        if (document.getElementById('contenedor-tareas-trz')) {
            if (contenedorTareasTrz.children.length > 0) {
                const hr = document.createElement('hr');
                hr.className = 'my-4'; 
                contenedorTareasTrz.appendChild(hr);
            }
        }

        if (document.getElementById('contenedor-tareas-trz')) {
            contenedorTareasTrz.appendChild(divTarea); 
        }
    ...
    // TAREA PAR
        ...
        <div class='row justify-content-center pb-4 fadeInUp-animation'>
            <div class=' ' style='width: 70%; box-shadow: 0px; background-color: #f8f9fa !important'>
                <div class='card-header'>
                    <div class='d-flex align-items-center justify-content-between mb-2'>
                        <h5><span class='badge badge-info'>#${id}</span> Crear Trazado</h5>
                        ...
                        <div class='col-md-6 mb-2' id='estado-container-${id}'>
                            ...
                            <div class='input-group input-group-sm'>
                                ...
                                <select id='estado-select-${id}' class='form-control d-none mr-2'>
                                    ${estados_trz.map(e => `<option value='${e}'>${e}</option>`).join('')}
                                </select>
...
function cambiarEstadoTrz(){  
    contadorTareasTrz++;
    crearTareaTrz(contadorTareasTrz); 
}

crearTareaTrz(contadorTareasTrz);


// ================== FIN TAREAS TRAZADOS - ESTRUCTURAL =========