...
// =================== TAREAS REVISION TROQUELES - ESTRUCTURAL ============
...
function crearTareaRt(id, autoFocus = false) {
    ...
    const estadosImpares = estados_rt.filter((_, index) => (index + 1) % 2 === 1);
    ...
    if (id % 2 === 1) {
        ...
        divTarea.innerHTML = `
            ${modalHTML}
            <div class='row justify-content-center info-box-content pb-4 fadeInUp-animation'>
                <div class='' style='width: 70%; box-shadow: 0px !important; background-color: #f8f9fa !important'>
                    <div class='card-header'>
                        <div class='d-flex align-items-center justify-content-between mb-2'>
                            <h5><span class='badge badge-info'>#${id}</span> Revisión de Troquel</h5>
                            ${botonModal}
                        </div>
                    </div>
                    <div class='card-body'>
        ...
        if (document.getElementById('contenedor-tareas-rt')) {
            if (contenedorTareasRt.children.length > 0) {
                const hr = document.createElement('hr');
                hr.className = 'my-4'; 
                contenedorTareasRt.appendChild(hr);
            }
        }

        if (document.getElementById('contenedor-tareas-rt')) {
            contenedorTareasRt.appendChild(divTarea); 
        }
        ...
        divTarea.innerHTML = `
            <div class='row justify-content-center pb-4 fadeInUp-animation'>
                <div class=' ' style='width: 70%; box-shadow: 0px; background-color: #f8f9fa !important'>
                    <div class='card-header'>
                        <div class='d-flex align-items-center justify-content-between mb-2'>
                            <h5><span class='badge badge-info'>#${id}</span> Revisión</h5>
                            ...
                            <div class='col-md-6 mb-2' id='estado-container-${id}'>
                                ...
                                <div class='input-group input-group-sm'>
                                    ...
                                    <select id='estado-select-${id}' class='form-control d-none mr-2'>
                                        ${estados_rt.map(e => `<option value='${e}'>${e}</option>`).join('')}
                                    </select>
...
function cambiarEstadoRt(){  
    contadorTareasRt++;
    crearTareaRt(contadorTareasRt); 
}

crearTareaRt(contadorTareasRt);


// ================== FIN TAREAS REVISION TROQUELES - ESTRUCTURAL =========